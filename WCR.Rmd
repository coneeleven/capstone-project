...
wine_converted_dates <- wine_clubgroupid.sales %>% 
  ...
  separate(cdate, c("year", "month", "day")) %>% 
...
```{r}
x <- wine_converted_dates$YM
z <- as.Date(x, "%Y-%m")
wine_converted_dates
z
```
...
ggplot(subset(wine_converted_dates, clubgroup_id.x = 27), aes(YM, revenue)) + geom_bar(stat = "identity") 
```
